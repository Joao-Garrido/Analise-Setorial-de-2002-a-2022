"""
=============================================================================
 ANÁLISE COMPLETA FINAL (SEM 2006)
 Risco Político em Anos Eleitorais no Brasil (2002, 2010–2022)
=============================================================================
...
OUTPUT_DIR = "./output_ff3_bhar_sem2006"
...
ANOS_PLACEBO = [2003, 2011, 2013, 2017, 2019]

DATAS_HGPE_PLACEBO = {
    2003: pd.Timestamp("2003-08-20"),
    # 2007 removido (correspondia ao placebo de 2006)
...
def gerar_grafico_linha_tempo(
    ...
    ax.set_title(
        ...
        "Janela de Antecipação: HGPE → Véspera do 1º Turno (Média 2002, 2010–2022)",
        fontsize=14,
    )
...
def gerar_visualizacoes(
    ...
    if not hm_data.empty:
        ...
        ax.set_title(
            ...
            "Anos Eleitorais 2002, 2010–2022 (excl. 2006)",
            fontsize=14,
        )
...
def gerar_texto_metodologia(output_dir: str):
    ...
    texto = """# Metodologia — Risco Político em Anos Eleitorais no Brasil (2002, 2010–2022) — SEM 2006
# Versão: Fama-French 3 Fatores + BHAR (excluindo eleição de 2006)
...
- Teste Placebo: anos não-eleitorais (2003, 2011, 2013, 2017, 2019)
...
def gerar_graficos_conclusao(df_eleitoral, output_dir):
    """
    Gera gráficos de síntese final: Ranking Setorial e Risco x Retorno.
    """
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    ...
    log.info("Gerando gráficos de conclusão (Ranking e Risco x Retorno)...")

    if df_eleitoral.empty:
        log.warning("  DataFrame eleitoral vazio — pulando gráficos de conclusão.")
        return

    # Agrupa por setor: Média (Retorno) e Desvio Padrão (Risco) entre eleições
    df_agg = df_eleitoral.groupby("setor")["abhar_vw"].agg(["mean", "std"]).reset_index()
    df_agg.columns = ["Setor", "Retorno_Medio", "Risco_Volatilidade"]

    # --- GRÁFICO 1: RANKING (Bar Chart Horizontal) ---
    plt.figure(figsize=(12, 6))
    df_agg = df_agg.sort_values("Retorno_Medio", ascending=True)

    # Cores: Verde (Lucro) vs Vermelho (Prejuízo)
    cores = ["#d62728" if x < 0 else "#2ca02c" for x in df_agg["Retorno_Medio"]]

    plt.barh(df_agg["Setor"], df_agg["Retorno_Medio"], color=cores, alpha=0.8)
    plt.axvline(0, color="black", linewidth=0.8, linestyle="--")

    # Formata labels de porcentagem
    for i, v in enumerate(df_agg["Retorno_Medio"]):
        plt.text(
            v + (0.002 if v > 0 else -0.002), i,
            f"{v:.1%}",
            va="center", ha="left" if v > 0 else "right",
            fontsize=9, fontweight="bold",
        )

    plt.title("Ranking Final: Quem Ganha/Perde na Média das Eleições? (2002, 2010-2022, excl. 2006)")
    plt.xlabel("Retorno Anormal Médio (ABHAR)")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "conclusao_ranking_setorial.png"), dpi=300, bbox_inches="tight")
    plt.close()
    log.info("  ✓ conclusao_ranking_setorial.png")

    # --- GRÁFICO 2: RISCO x RETORNO (Scatter Plot) ---
    plt.figure(figsize=(10, 8))
    sns.scatterplot(
        data=df_agg, x="Risco_Volatilidade", y="Retorno_Medio",
        hue="Setor", s=200, style="Setor", legend=False,
    )

    # Adicionar nomes dos setores
    for _, row in df_agg.iterrows():
        plt.text(
            row["Risco_Volatilidade"] + 0.002, row["Retorno_Medio"],
            row["Setor"], fontsize=9,
        )

    plt.axhline(0, color="black", linestyle="--", alpha=0.5)
    plt.axvline(df_agg["Risco_Volatilidade"].mean(), color="gray", linestyle=":", label="Risco Médio")

    plt.title("Mapa de Conclusão: Risco (Volatilidade) vs Retorno (Média)")
    plt.xlabel("Risco (Desvio Padrão entre Eleições)")
    plt.ylabel("Retorno Médio (ABHAR)")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "conclusao_risco_retorno.png"), dpi=300, bbox_inches="tight")
    plt.close()
    log.info("  ✓ conclusao_risco_retorno.png")
...
def main():
    ...
    print("\n" + "█" * 80)
    print("█                                                                              █")
    print("█   RISCO POLÍTICO EM ANOS ELEITORAIS NO BRASIL (2002, 2010–2022)            █")
    ...
    # Gráficos de Conclusão (Ranking + Risco x Retorno)
    gerar_graficos_conclusao(df_eleitoral, OUTPUT_DIR)
    ...
    # Resumo por setor
    if not df_eleitoral.empty:
        print("  RESUMO POR SETOR (Média dos anos eleitorais, excl. 2006):")
        print("  " + "-" * 76)
